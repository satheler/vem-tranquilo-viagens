                ...
                <div class="card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <a href="{{ route('segurofuncionario.index') }}" class="btn btn-sm btn-primary">{{ __('Voltar') }}</a>
                            </div>
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('segurofuncionario.store') }}" autocomplete="off">
                            @csrf
                            ...
                            <div class="pl-lg-4">
                                <div class="ms-container">
                                        <div class="row clearfix">
                                                <div class="col-md-6">
                                                    <label class="form-control-label text-right" for="input-empresa">{{ __('LISTA DE FUNCIONÁRIOS') }}</label>
                                                </div>
                                                <div class="col-md-6">
                                                    <label class="form-control-label text-right" for="input-empresa">{{ __('LISTA DE FUNCIONÁRIOS ASSEGURADOS') }}</label>
                                                </div>
                                            </div>

                                    <select multiple="multiple" name="user[]">
                                        @foreach ($lista["user"] as $item)
                                            <option value="{{ $item->id }}"><div class = "col-md-4">Nome: {{ $item->name}} - Função: {{ $item->tipo['nome'] }}  </div></option>
                                        @endforeach
                                    </select>
                                    @if ($errors->has('user'))
