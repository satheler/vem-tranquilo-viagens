                        ...
                        @foreach ($trajetos as $trajeto)
                            <li class="list-group-item px-0" style="background-color: transparent;">
                                <form method="POST" action="{{ route('page_compra.poltrona') }}">
                                    <input type="hidden" name="alocacao_id" value="{{ $trajeto['info']->id }}">
                                    <input type="hidden" name="trecho_origem_id" value="{{ $trajeto['info']->origem->id }}">
                                    <input type="hidden" name="trecho_destino_id" value="{{ $trajeto['info']->destino->id }}">
                                    <input type="hidden" name="valor" value="{{ $trajeto["valor"] }}">
                                    <div class="row align-items-center text-center">
                                        <div class="col">
                                            <h3 class="mb-0" data-toggle="tooltip" data-placement="top" title="Horário da partida">{{ $trajeto['info']->origem->horarioSaida }}</h3>
                                            <h3 class="mb-0" data-toggle="tooltip" data-placement="bottom" title="Hora da Chegada. Esta informação é apenas para efeito de orientação e não deve ser utilizada para compromissos agendados, pois, trata-se de apenas uma previsão de chegada ao destino.">{{ $trajeto['info']->destino->horarioSaida }}</h3>
                                        </div>
                                        <div class="col">
                                            <h5 class="mb-0" data-toggle="tooltip" data-placement="top" title="Tipo de ônibus">{{ $trajeto['onibus']->categoria->categoria }}</h5>
                                            <h5 class="mb-0">{0} assentos livres</h5>
                                        </div>
                                        <div class="col">
                                            <div class="row">
