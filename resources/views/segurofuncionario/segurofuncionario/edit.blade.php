        ...
        <div class="row">
            <div class="col-xl-12 order-xl-1">
                <div class="card bg-secondary shadow">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route('segurofuncionario.update', $lista['seguro']->id) }}" autocomplete="off">
                            @csrf
                            @method('put')
                            ...
                            <div class="pl-lg-4">
                                    <div class="ms-container">
                                        ...
                                        <select multiple="multiple" name="user[]">
                                            @foreach ($lista["user"] as $user)
                                                @foreach ($lista["seguro"]->funcionario as $segurados)
                                                    <option value="{{ $user->id }}" @if($user->id == $segurados->id) {{ "selected" }} @endif>
                                                        <div class="col-md-4">Nome: {{ $user->name }} - Função: {{ $user->tipo['nome'] }} </div>
                                                    </option>
                                                @endforeach
                                            @endforeach
                                        </select>
                                    ...
                                    {{-- <div class="col-md-4"> --}}
                                            ...
                                            {{-- <label class="form-control-label text-right" for="input-empresa">{{ __('LISTA DE ÔNIBUS ASSEGURADOS') }}</label> --}}
                                                {{-- @foreach ($lista["seguro"]->user as $seguro)
                                                <tr name="user[]">
